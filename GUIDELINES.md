...
**[CRITICAL]** AI ассистент должен проверять этот файл перед каждым ответом пользователю.
-->
...
2. **Деплой в dev-окружение**
   - Убедиться что все изменения закоммичены: `git status`
   - Добавить изменения: `git add .`
   - Создать коммит: `git commit -m "Описание изменений"`
   - Отправить изменения на GitHub: `git push origin dev`
   ...
   # Создать коммит
   git commit -m "Fix CallbackQuery import in new_generation.py"
   
   # Отправить изменения в dev ветку на GitHub
   git push origin dev
   
   # Переключиться на master ветку
   git checkout master
   
   # Смержить изменения из dev
   git merge dev --no-ff
   
   # Отправить изменения в Amvera
   git push amvera master
   ```

3. **Последовательность команд для деплоя**
   - Добавить изменения: `git add .`
   - Создать коммит: `git commit -m "Описание изменений"`
   - Отправить изменения в ветку dev: `git push origin dev`
   - Переключиться на master: `git checkout master`
   - Смержить изменения из dev: `git merge dev --no-ff`
   - Отправить изменения в Amvera: `git push amvera master`

   **Важные примечания:**
   - Все команды выполняются в терминале
   - Перед выполнением команд убедитесь что находитесь в корневой директории проекта
   - Для выполнения команд в Windows используйте PowerShell
   - Если возникают конфликты при merge, используйте `git mergetool` для их разрешения
   - После успешного деплоя проверьте статус приложения в панели управления Amvera
...
5. **Последовательность деплоя**
   ...
   - Добавить изменения: `git add .`
   - Создать коммит: `git commit -m "Описание изменений"`
   - Отправить изменения: `git push amvera master`
   - Дождаться автоматического деплоя
...
- **[CRITICAL]** Все коммуникации с пользователем должны осуществляться на русском языке
- Технические термины могут быть на английском языке, если это общепринятая практика
- Комментарии в коде могут быть как на русском, так и на английском языке
